...
export default defineComponent({
  ...
  computed: {
    lineSegments() {
      ...
      const computedTimes = this.store.computedTimes
      ...
      return lines.map((line, i) => {
        ...
        for (let set = 0; set < line.divisor; set++) {
          sets[set] = []
          const offsetTime = set * (this.store.monthLength / line.divisor)
          for (let j = 0; j < length; j++) {
            const currHalt = line.halts[j]
            const nextHalt = line.halts[(j+1) % length]
            const currStationIndex = this.store.findStationIndex(currHalt.stationId)
