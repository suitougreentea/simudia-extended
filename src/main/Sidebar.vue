...
export default defineComponent({
  ...
  methods: {
    ...
    changeLineVisibility(index, value) {
      this.store.modifyLine({ index: index, key: "visible", value })
    },
    changeHalt(key, value) {
      this.store.modifyLineHalt({ lineIndex: this.$parent.lineSelection.selectedLine, haltIndex: this.$parent.lineSelection.selectedHalt, key, value })
    },
    changeMonthLength(value) {
      this.store.modifyMonthLength({ value })
    },
    changeShiftDivisor(value) {
      this.store.modifyShiftDivisor({ value })
    },
    departureTimeShift(time) {
      const monthLength = this.store.monthLength
      const shiftDivisor = this.store.shiftDivisor
  ...
  computed: {
    ...
    nextHalt() {
      const halts = this.store.lines[this.$parent.lineSelection.selectedLine].halts
      return halts[(this.$parent.lineSelection.selectedHalt+1) % halts.length]
    },
    stationName() {
      return this.store.stations[this.store.findStationIndex(this.currentHalt.stationId)].name
    },
    nextStationName() {
      return this.store.stations[this.store.findStationIndex(this.nextHalt.stationId)].name
    },
    lineInfoString() {
      const result = []
      const monthLength = this.store.monthLength
      const shiftDivisor = this.store.shiftDivisor
      const line = this.store.lines[this.$parent.lineSelection.selectedLine]
      const halts = line.halts
      const times = this.store.computedTimes[this.$parent.lineSelection.selectedLine]
      for (let i = 0; i < halts.length; i++) {
        const halt = halts[i]
        const station = this.store.findStation(halt.stationId)
      ...
      const firstStation = this.store.findStation(firstHalt.stationId)
